...
english_alert_message1 = "Attention! Eyes closed for an extended period."
english_alert_message2 = "You are yawning a lot. Stop nearby and take some rest."
english_alert_message3 = "You are constantly blinking your eyes. Please take a stop."
...
COUNTER = 0
RAPID_BLINK_COUNTER = 0
RAPID_BLINK_THRESHOLD = 5
    ...
    for rect in rects:
        ...
        if ear < EYE_AR_THRESH:
            ...
            if COUNTER >= EYE_AR_CONSEC_FRAMES:
                ...
                sound.play()
                engine.say(english_alert_message1)
                engine.runAndWait()
            
            
            if RAPID_BLINK_COUNTER >= RAPID_BLINK_THRESHOLD:
                cv2.putText(frame, "Rapid Blinking!", (500, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                sound.play()
                engine.say(english_alert_message3)
                engine.runAndWait()
        ...
        if mar > MOUTH_AR_THRESH:
            cv2.putText(frame, "Yawning!", (800, 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            sound.play()
            engine.say(english_alert_message2)
            engine.runAndWait()
        else:
            #is playing = false
            pass
